# -*- coding: utf-8 -*-
"""Render statistics-papers/iss/ from iss_map_data.py.
...
OUT = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "iss") + os.sep
...
def index_page(percounts):
    ...
    b.append('  <p>The other maps on this site &mdash; '
             ...
             '<a href="../../ugc-net-statistics/index.html">UGC NET units</a> &mdash; '
    ...
    shell_iss.page(
        ...
        sub="Four statistics papers &middot; %d syllabus lines &middot; %d%% covered here "
            "&middot; %d gaps named" % (n, round(100.0 * (total["deep"] + total["brief"]) / n),
                                        total["missing"]),
        body="".join(b),
        back="../index.html", backlabel="Back to Statistics for Competitive Exams",
