...
Writes .html files into per-course directories under data-science/, mirroring
the layout of the Statistics half of the site:

    index.html                     course hub
    css/styles.css                 shared stylesheet
    <course-slug>/index.html
    <course-slug>/unit1.html ... unit5.html
    <course-slug>/practice.html, lab.html
...
# The generator now lives in tools/data-science/, three levels above the
# repository root -- but everything it writes belongs in the section, so ROOT
# stays the SECTION root and every "ROOT / ..." below is unchanged.
    ...
    for path, slug, title, tool, executed in sources:
        ...
        out.write_text(page(
            ...
            crumbs=f'<a href="../index.html">Home</a> &raquo; '
                   f'<a href="index.html">{html.escape(course["title"])}</a> '
                   f'&raquo; <a href="lab.html">Lab</a>',
            body=body,
            css_prefix="../",
            nav=[("\u2190 Back to the lab", f"lab.html")],
...
def lab_sources(course):
    ...
    for lab_dir in sorted((ROOT / LAB_DIR).iterdir()):
        ...
        for md_path in sorted(lab_dir.glob("*.md")):
            ...
            # The file no longer repeats its own folder: these pages sit in
            # data-science/<course>/, so "01_create_vm.html" is enough.
...
def build_course(course, link_map):
    ...
    for idx, (unit_title, unit_desc) in enumerate(course["units"], start=1):
        cards.append(
            f'    <a class="unit-card" href="unit{idx}.html">\n'
...
def main():
    ...
    for course in COURSES:
        ...
        for idx in range(1, 6):
            lm[f"unit-{idx}.md"] = f"unit{idx}.html"
        for fname, (out_slug, _, _) in EXTRA_PAGES.items():
            lm[fname] = f"{out_slug}.html"
        lm["README.md"] = f"index.html"
        for fname, out_slug, _, _ in TOP_PAGES:
