...
# The generators now sit beside this script, not under the section.
...
def main():
    ...
    for md in ROOT.glob("notes/**/*.md"):
        ...
        for mo in re.finditer(repo_path, body):
            ...
            n_links += 1
            # labs/, notes/, data/, docs/ and css/ are inside the section;
            # tools/ moved out of it in the restructure and is now named from
            # the repository root, the way a reader would type it.
            base = ROOT.parent if target.startswith("tools/") else ROOT
